                ...
                <div class="modal-body">
                        ...
                        <select class="form-select @error('cpu_component') is-invalid @enderror"
                            ...
                            @foreach($cpuComponents as $cpu_component)
                                <option value="{{ $cpu_component->id }}"
                                        @if(old('cpu_component') == $cpu_component->id) selected @endif>{{ $cpu_component->name }}</option>
                            @endforeach
                            @else
                                <option value="{{ $cpuComponent->id }}" selected>{{ $oldField('cpu_component') }}</option>
                            @endif
                        </select>
                    ...
                    <div class="form-floating mb-3">
                        <input type="number" class="form-control @error('cpu_quantity') is-invalid @enderror"
                               ...
                               max="1000" placeholder="Quantity" value="{{ old('cpu_quantity') ?? $oldField('cpu_quantity') }}">
                    ...
                    <div class="form-floating mb-3">
                        <input type="number" class="form-control @error('cpu_price') is-invalid @enderror"
                               ...
                               placeholder="Price" value="{{ old('cpu_price') ?? $oldField('cpu_price') }}">
                    ...
                    <div class="form-floating mb-3">
                        <textarea style="height: 250px"
                                  ...
                                  placeholder="Product Description">{{ old('cpu_description') ?? $oldField('cpu_description') }}</textarea>
