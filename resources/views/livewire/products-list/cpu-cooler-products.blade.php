...
{{--                                                    src="{{ $product_cpu_cooler->component->image_path }}"--}}
                                        ...
                                        <td>{{ $product_cpu_cooler->component->cpu_cooler->noise_level ?? null }}dB
                                        </td>
                                        <td>{{ $product_cpu_cooler->store->name }}</td>
                                        <td>&#8369; {{ number_format($product_cpu_cooler->price,2) }}</td>
                                        <td>
                                            <form action="{{ route('add_product', $product_cpu_cooler) }}"
                                                  method="post">
                                                @csrf
                    ...
                    @else
                        <p class="lead text-center">No Compatible CPU Coolers</p>
                    @endif
                </div>
            </div>
        </div>
